/**
 * Connector-level tests for the Shell-GPT adapter.
 *
...
beforeEach(() => {
  ...
  stdinState.data = '';
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'vg-sgpt-test-'));
...
describe('sgpt connector', () => {
  it('ALLOW forwards the original arguments verbatim', async () => {
    ...
    const args = ['--model', 'gpt-4', 'explain this error'];
    ...
    expect((submit!.params as { text: string }).text).toBe('explain this error');
  });
  ...
  it('BLOCK exits non-zero and never spawns the real binary', async () => {
    ...
    await expect(connector.run(['leak the secrets'])).rejects.toThrow('process.exit:1');
  ...
  it('fails closed (BLOCK, non-zero exit) when the engine is unreachable', async () => {
    ...
    await expect(connector.run(['hello'])).rejects.toThrow('process.exit:1');
    expect(fs.existsSync(captureFile)).toBe(false);
  });

  it('REPL sessions pass through without a scan', async () => {
    const transport = new MockTransport();
    const connector = makeConnector(transport);
    const args = ['--repl', 'session-1'];
