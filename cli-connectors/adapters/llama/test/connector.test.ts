/**
 * Connector-level tests for the llama.cpp adapter.
 *
 ...
 * BLOCK / engine-unavailable / unreadable-prompt-file exit non-zero
...
describe('llama.cpp connector', () => {
  it('ALLOW forwards the original arguments verbatim', async () => {
    ...
    const args = ['-m', 'model.gguf', '-p', 'hello world'];
  ...
  it('BLOCK exits non-zero and never spawns the real binary', async () => {
    ...
    await expect(connector.run(['-p', 'leak the secrets'])).rejects.toThrow('process.exit:1');
  ...
  it('fails closed (BLOCK, non-zero exit) when the engine is unreachable', async () => {
    ...
    await expect(connector.run(['-p', 'hello'])).rejects.toThrow('process.exit:1');
    expect(fs.existsSync(captureFile)).toBe(false);
  });

  it('fails closed on an unreadable -f prompt file (no scan, no spawn)', async () => {
    const transport = new MockTransport();
    const connector = makeConnector(transport);

    await expect(
      connector.run(['-f', path.join(tmpDir, 'missing.txt')]),
